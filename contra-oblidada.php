...
<?php
// variable per controlar si mostrem el formulari o el missatge
$correo_enviado = false;

// comprovem si el formulari s'ha enviat
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['email_reset'])) {
  ...
  <div class="login-container">
    <div class="login-card">
      ...
      <?php if ($correo_enviado): ?>
        ...
        <div style="background-color: #eafaf1; color: #27ae60; padding: 15px; border-radius: 8px; margin-bottom: 20px; font-size: 15px; text-align: center; border: 1px solid #c3e6cb;">
          si existeix un compte associat a aquest correu, rebràs un enllaç per restablir la teva contrasenya en uns minuts. si us plau, revisa també la teva carpeta de correu brossa.
        </div>
        
        <div class="links">
          <span class="create-account"><a href="accedir.php">tornar a iniciar sessió</a></span>
        </div>
        
      <?php else: ?>
        
        <p>introdueix el teu correu i t'enviarem les instruccions</p>
        
        <form method="POST" action="">
          <div class="form-group">
            <label for="email_reset">correu electrònic</label>
            <input type="email" id="email_reset" name="email_reset" placeholder="exemple@correu.com" required>
          </div>
          
          <button type="submit" class="btn-submit">enviar enllaç</button>
        </form>
