...
$sql_productes = "SELECT nombre_producto, descripcion, precio, imagen FROM productos";
$resultat_productes = @mysqli_query($conn, $sql_productes);
...
<head>
  ...
  <title>Productes | BrianBarber</title>
...
<main class="main-content-serveis">
  <div class="box-serveis-contenidor">
    <h2>Els nostres productes</h2>
    <p>Cuida la teva imatge també a casa amb els nostres productes de barberia clàssica i premium. Les millors marques a la teva disposició.</p>
    
    <div class="productes-grid">
      <?php if ($resultat_productes && mysqli_num_rows($resultat_productes) > 0) : ?>
        <?php while ($row = mysqli_fetch_assoc($resultat_productes)) : ?>

          <div class="producte-card">
            <div class="producte-img-container">
              <?php 
                $imgPath = $row['imagen'];
              ...
              ?>
              <img src="<?php echo htmlspecialchars($imgPath); ?>" alt="<?php echo htmlspecialchars($row['nombre_producto']); ?>">
            </div>
            <h3><?php echo $row['nombre_producto']; ?></h3>
            <p><?php echo $row['descripcion']; ?></p>
            <div class="servei-preu-info">
              <span class="opcio-tisora"><?php echo number_format($row['precio'], 2); ?>€</span>
            </div>
          </div>
        <?php endwhile; ?>
        <?php else : ?>
          <p>No hi ha productes disponibles en aquest moment. Torna més tard!</p>
        <?php endif; ?>
      </div>
